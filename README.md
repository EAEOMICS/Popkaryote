...
Usage Parameters:
  ...
  step                    Distance between start positions of windows
                          If not given, defaults to window size (non-overlapping) (default = None)
  ...
  gbff                    Path to gbff annotation file of the refernce genome:
...
usage example:
snakemake --cores {NUM_OF_CORES} --sdm conda --config output='{output_directory_full_path}' step=100 min_samples_per_pop=30 threads={NUM_OF_CORES} reference='{REF_GENOME_FULL_PATH}'
